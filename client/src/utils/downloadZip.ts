...
function buildReadmeForCline(projectName: string, screenshots: File[]): string {
  ...
  return [
    ...
    "3. Read handoff.md for target placement, screenshot structure authority, layout, visible actions, states, and unknowns.",
    ...
    "- target placement is missing",
    "- screenshot structure authority is missing",
    "- anti-hallucination rules are missing",
    "",
    "## Kaze Rules",
    "- Verify real exports from @pcs-security/kaze-ui-library before importing.",
    "- Prefer real unprefixed exports such as Button, TextField, Dropdown, Avatar, Typography, TextArea, and Swatch when confirmed.",
    ...
    "- Do not assume AntD subcomponent APIs such as Table.Row, Table.Cell, Steps.Step, Tabs.TabPane, Dropdown.Option, or Select.Option unless installed Kaze typings or existing usage confirms them.",
    ...
    "- Treat screenshots as the source of truth for visible page structure.",
    "- Replace blank, placeholder, scaffold-only, or structurally wrong output inside allowed files when it does not match the screenshot.",
...
const contradictionPatterns = [
  /Forbidden\\s+(?:Prefixed\\s+)?Names:\\s*.*\`?\\b(?:Button|TextField|Dropdown|Avatar|Typography|TextArea|Swatch)\\b\`?/i,
  /fake Kaze-prefixed components such as\\s+\`?\\b(?:Button|TextField|Dropdown|Avatar|Typography|TextArea|Swatch)\\b\`?/i,
  /Does not use fake Kaze-prefixed components such as\\s+\`?\\b(?:Button|TextField|Dropdown|Avatar|Typography|TextArea|Swatch)\\b\`?/i,
  /Does not import\\s+\`?\\b(?:Button|TextField|Dropdown|Avatar|Typography|TextArea|Swatch)\\b\`?/i,
  /\`?\\b(?:Button|TextField|Dropdown|Avatar|Typography|TextArea|Swatch)\\b\`?\\s+(?:is|are)\\s+(?:fake|invalid|wrong|forbidden)/i,
  /(?:fake|invalid|wrong|forbidden)\\s+(?:Kaze\\s+)?(?:exports?|names?|components?)\\s*[:\\-][^\\n]*\\b(?:Button|TextField|Dropdown|Avatar|Typography|TextArea|Swatch)\\b/i,
  /(?:\\*\\*)?(?:Wrong|Incorrect):(?:\\*\\*)?\\s*\`?import\\s*{[^}]*\\b(?:Button|TextField|Dropdown|Avatar|Typography|TextArea|Swatch)\\b[^}]*}/i,
  /Invalid:\\s*Do not use fake prefixed names like\\s*\`?\\b(?:Button|TextField|Dropdown|Avatar|Typography|TextArea|Swatch)\\b\`?/i,
];
const fakeKazeImportPattern =
  /import\\s*{\\s*[^}]*\\b(?:KazeButton|KazeInput|KazeSelect|KazeAvatar|KazeTypography)\\b[^}]*}\\s*from\\s*["']@pcs-security\\/kaze-ui-library["']/i;
const wrongMarkerPattern =
...
const forbiddenPatterns = [
  ...
  {
    file: "kaze-component-mapping.md",
    pattern: /(Forbidden Names|Forbidden Exports|Invalid)[^\\n]*(Do\\s+NOT\\s+use|Do\\s+not\\s+use|Never\\s+use)[^\\n]*\`?Button\`?[^\\n]*\`?TextField\`?[^\\n]*(\`?Dropdown\`?|\`?Avatar\`?|\`?Typography\`?|\`?TextArea\`?|\`?Swatch\`?)/i,
    message: "Real Kaze exports such as Button, TextField, Dropdown, Avatar, Typography, TextArea, and Swatch must not be listed as forbidden.",
  },
  {
    file: "kaze-component-mapping.md",
  ...
  {
    file: "kaze-component-mapping.md",
    pattern: /import\\s*{[^}]*\\b(?:TextAreaField|ColourSwatch)\\b[^}]*}\\s*from\\s*["']@pcs-security\\/kaze-ui-library["']/i,
    message: "Mapping must not import internal Kaze names TextAreaField or ColourSwatch as public exports.",
    allowIfAlsoMatches: /WRONG|Incorrect|do not import|invalid/i,
  },
];
...
const requiredTextChecks = [
  ...
  },
  {
    ...
    message: "Implementation prompt must include Target Placement.",
  },
  {
    file: "cline-implementation-prompt.md",
    text: "Screenshot Structure Authority",
    message: "Implementation prompt must include Screenshot Structure Authority.",
  },
  {
    file: "cline-implementation-prompt.md",
    text: "Validation Requirements",
    message: "Implementation prompt must include Validation Requirements.",
  },
  {
    file: "cline-implementation-prompt.md",
  ...
  {
    file: "kaze-component-mapping.md",
    text: "Confirmed Public Kaze Exports",
    message: "Mapping must include Confirmed Public Kaze Exports.",
  },
  {
    file: "handoff.md",
    text: "Target Placement",
    message: "Handoff must include Target Placement.",
  },
  {
    file: "handoff.md",
    text: "Screenshot Structure Authority",
    message: "Handoff must include Screenshot Structure Authority.",
  },
  {
    file: "handoff.md",
    text: "Required Content and Section Order",
    message: "Handoff must include Required Content and Section Order.",
  },
  {
    file: "kaze-component-mapping.md",
    text: "KazeButton",
  ...
  },
  {
    file: "qa-checklist.md",
    text: "Validation Requirements",
    message: "QA checklist must include Validation Requirements section.",
  },
  {
    file: "qa-checklist.md",
    text: "Accessibility",
    message: "QA checklist must include Accessibility section.",
  },
];
...
export function buildClineReadinessStandard(): string {
  return [
    ...
    "- Handoff includes exact target placement and allowed files.",
    "- Handoff states that the screenshot is the source of truth for visible page structure.",
    "- Handoff lists required visible structure and section order from the screenshot.",
    "- Cline prompt gives implementation sequence.",
    "- Kaze mapping lists real allowed components.",
    "- Kaze mapping uses public exports such as `TextArea` and `Swatch`, not internal names.",
    "- Fake Kaze-prefixed components are clearly forbidden.",
    "- Target placement rule exists.",
    "- Screenshot structure authority rule exists.",
    "- Anti-hallucination rules exist.",
    "- QA checklist covers visual, Kaze usage, implementation safety, validation, and accessibility.",
    ...
    "- The pack imports `TextAreaField` or `ColourSwatch` as public Kaze exports.",
